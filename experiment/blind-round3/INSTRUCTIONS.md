# Blind pair, round 3 — does PLACEMENT of the named finding move the citation?

Round 2 (September 2026) reproduced round 1 question for question on this pair: zero flips across
three side-swapped runs. Since then one of the two pages has been edited again. This round tests
that edit. Read this whole file before judging anything.

## What you are doing

For each of the three run folders below, you are given two pages that answer the same reader
...
**Judge all three as if they were unrelated.** Do not carry an opinion from one run into the next,
...
Write `verdict-round3.json` into each run folder, in exactly this shape:
...
Same judge and effort as round 2: Kimi K3, medium effort. **Budget cap: $3 for all three runs.**
If you approach it, stop and report how many runs completed.

## When you are done

Reply with only: the three `citation_score` values, and the count of questions you marked
